...
export default function LinksTable({  onDelete, reload }) {
  ...
  const safeLinks = Array.isArray(links)
    ? links.filter((x) => x && typeof x.code === "string")
    : [];
...
return (
    <div className="w-full max-w-4xl mx-auto bg-white border border-gray-200 shadow-xl rounded-2xl p-6 mt-10 space-y-6">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="text-gray-700 bg-gray-100 border-b">
              <th className="p-4 text-sm font-semibold text-left">Code</th>
              <th className="p-4 text-sm font-semibold text-left">Target</th>
              <th className="p-4 text-sm font-semibold text-left">Clicks</th>
          ...
          <tbody>
            {paginated.map((row) => (
              ...
              >
                ...
                {/* CLICKS */}
                <td className="p-4 font-semibold">{row.clicks}</td>
